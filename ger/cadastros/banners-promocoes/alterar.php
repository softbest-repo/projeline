	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				$sqlNomePagamento = "SELECT codBannerPromocao, nomeBannerPromocao, statusBannerPromocao FROM bannersPromocoes WHERE codBannerPromocao = '".$url[6]."' LIMIT 0,1";
...
?>
				<div id="localizacao-topo">
					<div id="conteudo-localizacao-topo">
						...
						<p class="flexa"></p>
						<p class="nome-lista"><?php echo $dadosNomePagamento['nomeBannerPromocao'] ;?></p>
						<br class="clear" />
					</div>
					<table class="tabela-interno" >
<?php
				if($dadosNomePagamento['statusBannerPromocao'] == "T"){
					$status = "status-ativo";
					$statusIcone = "ativado";
...
?>	
						<tr class="tr-interno">
							<td class="botoes-interno"><a href='<?php echo $configUrl; ?>cadastros/banners-promocoes/ativacao/<?php echo $dadosNomePagamento['codBannerPromocao'] ?>/' title='Deseja <?php echo $statusPergunta ?> o banner capa <?php echo $dadosNomePagamento['nomeBannerPromocao'] ?>?' ><img src="<?php echo $configUrl; ?>f/i/default/corpo-default/<?php echo $status ?>-branco.gif" alt="icone"></a></td>
							<td class="botoes-interno"><a href='javascript: confirmaExclusao(<?php echo $dadosNomePagamento['codBannerPromocao'] ?>, "<?php echo htmlspecialchars($dadosNomePagamento['nomeBannerPromocao']) ?>");' title='Deseja excluir o banner capa <?php echo $dadosNomePagamento['nomeBannerPromocao'] ?>?' ><img src='<?php echo $configUrl; ?>f/i/default/corpo-default/excluir-branco.gif' alt="icone"></a></td>
						</tr>
						<script>
...
<?php
				if(isset($_POST['alterar'])){
					...
					$sql = "UPDATE bannersPromocoes SET nomeBannerPromocao = '".preparaNome($_POST['nome'])."', tituloBannerPromocao = '".$_POST['titulo']."', fraseBannerPromocao = '".$_POST['frase']."', linkBannerPromocao = '".str_replace("\"", "&quot;", str_replace("'", "&#39;", $_POST['link']))."', novaAbaBannerPromocao = '".$_POST['novaAba']."', urlBannerPromocao = '".$urlBannerPromocao."' WHERE codBannerPromocao = '".$url[6]."'";
					...
					$sql = "SELECT * FROM bannersPromocoes WHERE codBannerPromocao = ".$url[6];
					$result = $conn->query($sql);
					$dadosBannerPromocao = $result->fetch_assoc();
					$_SESSION['nome'] = $dadosBannerPromocao['nomeBannerPromocao'];
					$_SESSION['titulo'] = $dadosBannerPromocao['tituloBannerPromocao'];
					$_SESSION['frase'] = $dadosBannerPromocao['fraseBannerPromocao'];
					$_SESSION['link'] = $dadosBannerPromocao['linkBannerPromocao'];
					$_SESSION['novaAba'] = $dadosBannerPromocao['novaAbaBannerPromocao'];
				}
...
?>				
						...
						<form action="<?php echo $configUrlGer; ?>cadastros/banners-promocoes/alterar/<?php echo $url[6] ;?>/" method="post">
