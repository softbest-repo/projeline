		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					...
					$sqlCons = "SELECT nomeComparativo, statusComparativo FROM comparativos WHERE codComparativo = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
				
					if($dadosCons['statusComparativo'] == "T"){
					...
					$sql =  "UPDATE comparativos SET statusComparativo = '".$alteraStatus."' WHERE codComparativo = ".$url[6];
