		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					...
					$sqlCons = "SELECT nomeBanner, statusBanner FROM banners WHERE codBanner = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
				
					if($dadosCons['statusBanner'] == "T"){
					...
					$sql =  "UPDATE banners SET statusBanner = '".$alteraStatus."' WHERE codBanner = ".$url[6];
