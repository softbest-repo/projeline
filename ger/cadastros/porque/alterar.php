	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				$sqlNomePagamento = "SELECT codPorque, nomePorque, statusPorque FROM porque WHERE codPorque = '".$url[6]."' LIMIT 0,1";
...
?>
				<div id="localizacao-topo">
					<div id="conteudo-localizacao-topo">
						...
						<p class="flexa"></p>
						<p class="nome-lista">Por que a Projeline</p>
						<p class="flexa"></p>
						<p class="nome-lista">Alterar</p>
						<p class="flexa"></p>
						<p class="nome-lista"><?php echo $dadosNomePagamento['nomePorque'] ;?></p>
						<br class="clear" />
					</div>
					<table class="tabela-interno" >
<?php
				if($dadosNomePagamento['statusPorque'] == "T"){
					$status = "status-ativo";
					$statusIcone = "ativado";
...
?>	
						<tr class="tr-interno">
							<td class="botoes-interno"><a href='<?php echo $configUrl; ?>cadastros/porque/ativacao/<?php echo $dadosNomePagamento['codPorque'] ?>/' title='Deseja <?php echo $statusPergunta ?> a área de atuação <?php echo $dadosNomePagamento['nomePorque'] ?>?' ><img src="<?php echo $configUrl; ?>f/i/default/corpo-default/<?php echo $status ?>-branco.gif" alt="icone"></a></td>
							<td class="botoes-interno"><a href='javascript: confirmaExclusao(<?php echo $dadosNomePagamento['codPorque'] ?>, "<?php echo htmlspecialchars($dadosNomePagamento['nomePorque']) ?>");' title='Deseja excluir a área de atuação <?php echo $dadosNomePagamento['nomePorque'] ?>?' ><img src='<?php echo $configUrl; ?>f/i/default/corpo-default/excluir-branco.gif' alt="icone"></a></td>
						</tr>
						<script>
...
<?php
				if(isset($_POST['alterar'])){
					...
					$sql = "UPDATE porque SET nomePorque = '".preparaNome($_POST['nome'])."', descricaoPorque = '".str_replace("'", "&#39;", $descricao)."', urlPorque = '".$urlPorque."' WHERE codPorque = '".$url[6]."'";
					...
					$sql = "SELECT * FROM porque WHERE codPorque = ".$url[6];
					$result = $conn->query($sql);
					$dadosPorque = $result->fetch_assoc();
					$_SESSION['nome'] = $dadosPorque['nomePorque'];
					$_SESSION['descricao'] = $dadosPorque['descricaoPorque'];
					$_SESSION['status'] = $dadosPorque['statusPorque'];
				}
...
?>				
						...
						<form action="<?php echo $configUrlGer; ?>cadastros/porque/alterar/<?php echo $url[6] ;?>/" method="post">
