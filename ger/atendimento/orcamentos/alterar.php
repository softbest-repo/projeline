	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				$sqlNomePagamento = "SELECT codOrcamento, nomeOrcamento, statusOrcamento FROM orcamentos WHERE codOrcamento = '".$url[6]."' LIMIT 0,1";
...
?>
				<div id="localizacao-topo">
					<div id="conteudo-localizacao-topo">
						...
						<p class="flexa"></p>
						<p class="nome-lista"><?php echo $dadosNomePagamento['nomeOrcamento'] ;?></p>
						<br class="clear" />
					</div>
					<table class="tabela-interno" >
<?php
				if($dadosNomePagamento['statusOrcamento'] == "T"){
					$status = "status-ativo";
					$statusIcone = "ativado";
...
?>	
						<tr class="tr-interno">
							<td class="botoes-interno"><a href='<?php echo $configUrl; ?>atendimento/orcamentos/ativacao/<?php echo $dadosNomePagamento['codOrcamento'] ?>/' title='Deseja <?php echo $statusPergunta ?> o integrante da orcamentos <?php echo $dadosNomePagamento['nomeOrcamento'] ?>?' ><img src="<?php echo $configUrl; ?>f/i/default/corpo-default/<?php echo $status ?>-branco.gif" alt="icone"></a></td>
							<td class="botoes-interno"><a href='javascript: confirmaExclusao(<?php echo $dadosNomePagamento['codOrcamento'] ?>, "<?php echo htmlspecialchars($dadosNomePagamento['nomeOrcamento']) ?>");' title='Deseja excluir o integrante da orcamentos <?php echo $dadosNomePagamento['nomeOrcamento'] ?>?' ><img src='<?php echo $configUrl; ?>f/i/default/corpo-default/excluir-branco.gif' alt="icone"></a></td>
						</tr>
						<script>
...
<?php
				if(isset($_POST['alterar'])){
					...
					$sql = "UPDATE orcamentos SET nomeOrcamento = '".preparaNome($_POST['nome'])."', subNomeOrcamento = '".preparaNome($_POST['subNome'])."', descricaoOrcamento = '".str_replace("'", "&#39;", $descricao)."', urlOrcamento = '".$urlOrcamento."' WHERE codOrcamento = '".$url[6]."'";
					...
					if($result == 1){
						...
						$erroData = "<p class='erro'>Problemas ao alterar integrante da orcamentos!</p>";
					}
				}else{
					$sql = "SELECT * FROM orcamentos WHERE codOrcamento = ".$url[6];
					$result = $conn->query($sql);
					$dadosOrcamento = $result->fetch_assoc();
					$_SESSION['nome'] = $dadosOrcamento['nomeOrcamento'];
					$_SESSION['subNome'] = $dadosOrcamento['subNomeOrcamento'];
					$_SESSION['descricao'] = $dadosOrcamento['descricaoOrcamento'];
					$_SESSION['status'] = $dadosOrcamento['statusOrcamento'];
				}
...
?>
						...
						<form action="<?php echo $configUrlGer; ?>atendimento/orcamentos/alterar/<?php echo $url[6] ;?>/" method="post">
