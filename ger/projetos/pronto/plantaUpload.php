<?php
	...
	use Intervention\Image\ImageManagerStatic as Image;
	 ...
	 $pastaDestino = $_SERVER['DOCUMENT_ROOT'].$urlUpload.'/f/plantas/';
	...
	foreach ($_FILES['arquivo']['tmp_name'] as $index => $tmp_name) {
		...
		if(in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'mp4'])){	
			...
			$sqlCont = "SELECT * FROM plantasImagens WHERE codProjeto = ".$codProjeto." ORDER BY ordenacaoPlantaImagem DESC LIMIT 0,1";
			$resultCont = $conn->query($sqlCont);
			$dadosCont = $resultCont->fetch_assoc();
			
			if($dadosCont['codPlantaImagem'] == ""){
				$cont = 1;
			}else{
				$cont = $dadosCont['ordenacaoPlantaImagem'] + 1;
			}
							
			$sqlProjeto = "INSERT INTO plantasImagens VALUES(0, ".$codProjeto.", ".$cont.", '".$ext."')";
			$resultProjeto = $conn->query($sqlProjeto);	

			$sqlPegaProjeto = "SELECT codPlantaImagem FROM plantasImagens ORDER BY codPlantaImagem DESC LIMIT 0,1";
			$resultPegaProjeto = $conn->query($sqlPegaProjeto);
			$dadosPegaProjeto = $resultPegaProjeto->fetch_assoc();
						
			$codPlantaImagem = $dadosPegaProjeto['codPlantaImagem'];
				
			move_uploaded_file($tmp_name, $pastaDestino.$codProjeto."-".$codPlantaImagem."-O.".$ext);
							
			chmod($pastaDestino.$codProjeto."-".$codPlantaImagem."-O.".$ext, 0755);
			
			if($ext != "mp4"){
			
				$imagem_original = $pastaDestino.$codProjeto."-".$codPlantaImagem."-O.".$ext;
				...
				if($liberado == "ok"){
					...
					$imagemWebP = $pastaDestino.$codProjeto."-".$codPlantaImagem."-W.webp";
					...
					switch ($ext) {
						case 'jpg':
						case 'jpeg':
						imagejpeg($new_image, $pastaDestino.$codProjeto."-".$codPlantaImagem."-MD.".$ext, 100);
						break;
						case 'png':
						imagepng($new_image, $pastaDestino.$codProjeto."-".$codPlantaImagem."-MD.".$ext);
						break;
						case 'gif':
						imagegif($new_image, $pastaDestino.$codProjeto."-".$codPlantaImagem."-MD.".$ext);
						break;
					}

					saveWebPImage($new_image, $imagemWebP, 98);

					chmod($pastaDestino.$codProjeto."-".$codPlantaImagem."-MD.".$ext, 0755);
					chmod($pastaDestino.$codProjeto."-".$codPlantaImagem."-W.webp", 0755);

					imagedestroy($new_image);
					imagedestroy($watermark);
				}else{							   
					$sqlDelete = "DELETE FROM plantasImagens WHERE codPlantaImagem = ".$codPlantaImagem;
	...
	echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrl."projetos/pronto/plantaImagens/".$codProjeto."/'>";				
?>
