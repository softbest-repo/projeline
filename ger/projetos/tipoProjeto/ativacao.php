	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					$sqlCons = "SELECT nomeTipoProjeto, statusTipoProjeto FROM tipoProjeto WHERE codTipoProjeto = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
					
					if($dadosCons['statusTipoProjeto'] == "T"){
					...
					$sql =  "UPDATE tipoProjeto SET statusTipoProjeto = '".$alteraStatus."' WHERE codTipoProjeto = ".$url[6];
					$result = $conn->query($sql);
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeTipoProjeto'];
