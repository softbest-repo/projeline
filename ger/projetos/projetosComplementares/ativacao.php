		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					...
					$sqlCons = "SELECT nomeProjetoComplementar, statusProjetoComplementar FROM projetosComplementares WHERE codProjetoComplementar = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
				
					if($dadosCons['statusProjetoComplementar'] == "T"){
					...
					$sql =  "UPDATE projetosComplementares SET statusProjetoComplementar = '".$alteraStatus."' WHERE codProjetoComplementar = ".$url[6];
					$result = $conn->query($sql);
				
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeProjetoComplementar'];
